...
def get_title_display(title: str, year: int, url: str) -> str:
    ...
    if version:
        title_display = "{}, {}, v{}".format(title, year, version.group(1))
    else:
        title_display = "{}, {}".format(title, year)
...
def get_yearly_url_imgs(year: int) -> list:
    ...
    url = "{}{}/std.html".format(URL_IMPAWARDS, year)
    ...
    for itr, tr in enumerate(trs[::2]):
        ...
        url_imgs = [
            format_url.format(base=URL_IMPAWARDS, year=year, link=x) for x in html_links
        ]
        ...
        dict_tmp = [
            ...
            for x, y, z, w in zip(path_imgs, path_thumbs, title_displays, url_imgs)
        ]

        append_to_jsonl("main_dataset.jsonl", dict_tmp)

        dict_imgs += dict_tmp
        print(f"Year {year} - {itr}/{len(trs)//2} done")
...
def download_poster(link: str, size_thumb: tuple = (100, 100)) -> tuple:
    ...
    img_poster.thumbnail(size_thumb, Image.Resampling.LANCZOS)
    img_poster.save(path_thumb)
    print(f"Downloaded {link} : {path_img}")
...
def main(argv):
    ...
    years = range(
        config["scraping"]["years_range"][0], config["scraping"]["years_range"][1] + 1
    )
    ...
    with open("yearly_urls.pkl", "wb") as f:
        pickle.dump(yearly_urls, f)

    with open("yearly_urls.json", "w") as f:
        json.dump(yearly_urls, f, indent=4)
