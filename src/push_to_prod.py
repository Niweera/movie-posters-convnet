...
def copy_db_dev_prod(uri_db_dev, uri_db_prod):
    ...
    data_dev = db_dev.query(
        Poster.id, Poster.closest_posters, Poster.title_display
    ).all()
    data_prod = [PosterWeb(x.id, x.closest_posters, x.title_display) for x in data_dev]
...
def create_db_prod(uri_db_dev, uri_db_prod, path_img, path_thumb):
    ...
    data_posters = db_dev.query(Poster.id, Poster.base64_img, Poster.base64_thumb).all()
