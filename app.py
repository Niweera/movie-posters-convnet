...
def cache_key():
    ...
    key = (
        ...
        + "?"
        + urlencode(
            [(k, v) for k in sorted(args) for v in sorted(args.getlist(k))], doseq=True
        )
    )
    return key
...
class ApiPosters(Resource):
    ...
    def get(self, id):
        ...
        if id == "idmovies":
            data = {
                x[1]: x[0] for x in self.db.query(Poster.id, Poster.title_display).all()
            }
        elif id == "2d":
            fields = (Poster.id, Poster.features_pca, Poster.path_thumb)
            ...
            idx_rnd = np.random.choice(range(n_data), 1500, replace=True)
            all_data = my_query.all()
            rnd_data = [all_data[i] for i in idx_rnd]
            data = [{"id": x[0], "xy": list(x[1]), "thumb": x[2]} for x in rnd_data]
        else:
            id = int(id)
            ...
            fields = (Poster.id, Poster.title_display, Poster.path_img)
...
if __name__ == "__main__":
    appli.run(host="0.0.0.0", port=8080, debug=True)
